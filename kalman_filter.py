...
class kalman_filter:
    ...
    def jacobian_A(self):
        ...
        return np.array([
            #x, y, th, w, v, vdot
            [1, 0, -v.item() * np.sin(th.item()) * dt, 0, np.cos(th.item()) * dt, 0], # partial deriv of x w.r.t state variables
            [0, 1, v.item() * np.cos(th.item()) * dt, 0, np.sin(th.item()) * dt,  0], # partial deriv of y w.r.t state variables
    ...
    def jacobian_H(self):
        ...
        return np.array([
            ...
            [0, 0, 0, v.item(), w.item(), 0], # partial deriv of ay w.r.t state variables
        ])
